...
module.exports = db => {
  router.get("/goals", async (req, res) => {
    try {
      ...
      db.query(
        `SELECT id, goal_name, user_id, start_date, to_char(start_date,'FMMonth FMDDth, YYYY') as simple_start_date, end_date, to_char(end_date,'FMMonth FMDDth, YYYY') as simple_end_date, cron, friend_1_phone_number, friend_2_phone_number FROM goals WHERE user_id = $1 ORDER BY goals.id`,
        [req.session.userId]
      ).then(({ rows: goals }) => {
        res.json(
          goals.reduce(
    ...
    try {
      ...
      const getDateArr = () => {
        ...
        for (
          ...
          nagDate.setDate(nagDate.getDate() + 1)
        ) {
      ...
      const nagQuery = arr => {
        ...
        arr.forEach(item => {
          valuesQuery =
            valuesQuery +
            `(${goalQuery.rows[0].id}, '${newGoal.nag}', null, ` +
            `'${item}'` +
            endValues;
        });
  ...
  router.put("/goals/edit", async (req, res) => {
    ...
    const finalNagUpdateQuery = (startQuery, endQuery, passinDateArray) => {
      ...
      passinDateArray.forEach(item => {
        middleNagUpdateQuery =
          middleNagUpdateQuery +
          `(${req.body.goalId}, '${req.body.nag}', null, ` +
          `'${item}'` +
          endQuery;
      });
    ...
    let actualFinalNagUpdateQuery = finalNagUpdateQuery(
      startNagUpdateQuery,
      endNagUpdateQuery,
      dateArray
    );
    actualFinalNagUpdateQuery = actualFinalNagUpdateQuery.substr(
      0,
